...
Reverse-engineered specification of [`js/webrtc-babycam.js`](../js/webrtc-babycam.js).
...
| Layer | Class | Role |
|-------|-------|------|
| **Model / controller** | `WebRTCsession` ([js/webrtc-babycam.js:14](../js/webrtc-babycam.js)) | Owns the WebRTC call, the snapshot image, statistics, status, and the master watchdog loop. One instance is shared by all cards that resolve to the same key. |
| **View / custom element** | `WebRTCbabycam extends HTMLElement` ([js/webrtc-babycam.js:1396](../js/webrtc-babycam.js)) | The `<webrtc-babycam>` Lovelace card. Renders a Shadow DOM (`<video>`, snapshot `<img>`, PTZ, shortcuts, state icon, header, debug log) and reacts to session + media events. |
| **Signaling transport** | `SignalingChannel` and subclasses (`Go2RtcSignalingChannel`, `WhepSignalingChannel`, `RTSPtoWebSignalingChannel`) ([js/webrtc-babycam.js:3561+](../js/webrtc-babycam.js)) | Pluggable offer/answer/ICE transport selected by `config.url_type`. |
...
- **REQ-ARCH-1** — Sessions are keyed by `WebRTCsession.key(config)`: the entity id
  ...
  `-v` when `video===false` ([:94](../js/webrtc-babycam.js)). Cards with identical
...
- **REQ-ARCH-2** — `WebRTCsession.getInstance(config)` returns the existing session
  for a key or lazily creates one and registers it ([:104](../js/webrtc-babycam.js)).
...
The session owns an `EventTarget` ([:62](../js/webrtc-babycam.js)). Each attached
card subscribes its `sessionEvent` handler to this fixed set of event types
([:604](../js/webrtc-babycam.js), dispatched throughout; handled at
[:2031](../js/webrtc-babycam.js)):
...
- **REQ-ARCH-5** — Attaching a card subscribes exactly these event types; detaching
  must unsubscribe the same set ([:619](../js/webrtc-babycam.js),
  [:653](../js/webrtc-babycam.js)). No card-side polling of session internals.

---

## 1. Runtime state & vocabulary

### 1.1 The `call` object ([:834](../js/webrtc-babycam.js))
...
### 1.2 Session status values (`setStatus`, [:751](../js/webrtc-babycam.js))
...
- **REQ-STATE-1** — `setStatus` must be idempotent: re-setting the current value
  emits nothing ([:752](../js/webrtc-babycam.js)).
...
- `isPlaying` / `isPlayingVideo` ([:2318](../js/webrtc-babycam.js))
- `isMediaStale` ([:2330](../js/webrtc-babycam.js))
- `isPlayingActive` = playing && !stale ([:2335](../js/webrtc-babycam.js))

### 1.4 Timing constants ([:16–:38](../js/webrtc-babycam.js))
...
Config defaults ([:2784](../js/webrtc-babycam.js)): `image_interval=3000`,
...
**Flow** (`setConfig` [:2773](../js/webrtc-babycam.js) → `connectedCallback`
[:3073](../js/webrtc-babycam.js)):

1. `setConfig` validates WebRTC support, required `url`+`entity`, and `ptz.service`;
...
2. `connectedCallback` runs one-time `globalInit()` (document gesture/keyboard
   listeners, [:1942](../js/webrtc-babycam.js)), then `render()` (destructive Shadow
   DOM rebuild, [:3037](../js/webrtc-babycam.js)) and
   `setupVisibilityAndResizeHandlers()` ([:2982](../js/webrtc-babycam.js)).
3. An `IntersectionObserver` (threshold 0) and a document `visibilitychange` listener
   ...
   ([:2843](../js/webrtc-babycam.js)).

**Requirements**

- **REQ-LIFE-1** — A card with no config must defer all work; `connectedCallback`
  returns early until `setConfig` provides one ([:3077](../js/webrtc-babycam.js)).
- **REQ-LIFE-2** — `setConfig` must throw on: missing `RTCPeerConnection` when
  audio/video enabled, missing `url`/`entity`, or `ptz` without `service`
  ([:2774](../js/webrtc-babycam.js)).
...
- **REQ-LIFE-4** — Becoming hidden must detach the card from the session, stop the
  ...
  designated background card (see §11) ([:2895](../js/webrtc-babycam.js)).
- **REQ-LIFE-5** — `disconnectedCallback` must remove all observers/listeners and
  apply invisibility ([:3095](../js/webrtc-babycam.js)).
...
- **REQ-LIFE-7** — Visibility uses both `IntersectionObserver` and a geometric
  ...
  [:2921](../js/webrtc-babycam.js)); when the tab is hidden the card is treated as not
  visible ([:2958](../js/webrtc-babycam.js)).

---

## 3. Pathway: Session attach / detach / termination

**Flow:** `attachCard` ([:584](../js/webrtc-babycam.js)) /
`detachCard` ([:632](../js/webrtc-babycam.js)) / `terminate` ([:563](../js/webrtc-babycam.js)).
...
- **REQ-SESS-1** — `attachCard` must add the card to `state.cards`, subscribe the
  ...
  visible or background mode is on ([:600](../js/webrtc-babycam.js)).
- **REQ-SESS-2** — Attaching aborts any pending session termination timer
  ([:592](../js/webrtc-babycam.js)).
- **REQ-SESS-3** — In background mode, attaching a card must release other
  ...
  ([:588](../js/webrtc-babycam.js), `releaseOtherBackgroundCards` [:2834](../js/webrtc-babycam.js)).
- **REQ-SESS-4** — `detachCard` removes the card and its listeners; if cards remain,
  the session keeps running ([:657](../js/webrtc-babycam.js)).
- **REQ-SESS-5** — When the last card detaches, termination must be **deferred** by
  ...
  grace window ([:665](../js/webrtc-babycam.js)). This prevents teardown during quick
...
([:450](../js/webrtc-babycam.js)) is a single self-rescheduling timer chain.
...
1. **Re-entrancy guard:** proceed only if `id === watchdogTimeoutId` (the scheduled
   ...
   single live watchdog chain ([:451](../js/webrtc-babycam.js)).
2. A fresh start (no `id`) resets status to `reset` and clears stats
   ([:466](../js/webrtc-babycam.js)).
...
- **REQ-WD-1** — Exactly one watchdog timer chain runs per session at a time;
  overlapping `play()` invocations are no-ops ([:451](../js/webrtc-babycam.js)).
- **REQ-WD-2** — The watchdog must reschedule itself in `finally` on every tick
  ...
  ([:514](../js/webrtc-babycam.js)). The only exit is a terminated session with zero
  cards ([:515](../js/webrtc-babycam.js)).
- **REQ-WD-3** — `reconnectDate` is the liveness deadline. While the stream renders,
  ...
  long the call is torn down and re-established ([:488](../js/webrtc-babycam.js)).
  This is the *retry-forever* mechanism.
- **REQ-WD-4** — `extendCallTimeout` only ever moves the deadline later (monotonic);
  `timeoutCall` sets it to `0` to force reconnect ([:536](../js/webrtc-babycam.js)).
- **REQ-WD-5** — Each tick emits a `heartbeat` event carrying `live`, where `live`
  ...
  ([:506](../js/webrtc-babycam.js)).
- **REQ-WD-6** — `isStreaming` requires ICE state `connected|completed`, a remote
  stream, and at least one `live` track ([:787](../js/webrtc-babycam.js)).
...
([:819](../js/webrtc-babycam.js)) → `openSignalingChannel`
([:1112](../js/webrtc-babycam.js)) → `createPeer` ([:977](../js/webrtc-babycam.js)) →
negotiation.
...
3. If `microphone` and secure context, `getUserMedia({audio})` for uplink
   (failure is non-fatal) ([:853](../js/webrtc-babycam.js)).
...
5. `createPeer`: build `RTCPeerConnection` (STUN `stun.l.google.com:19302`) and wire
   handlers ([:988](../js/webrtc-babycam.js)).
...
7. Adding media triggers `onnegotiationneeded` → create offer (`iceRestart:true`) →
   `setLocalDescription` → `signalingChannel.sendOffer` ([:991](../js/webrtc-babycam.js)).
8. Answer arrives → `setRemoteDescription` → flush queued ICE candidates
   ([:1218](../js/webrtc-babycam.js)).
9. ICE candidates exchanged both directions; local candidates sent as gathered,
   ...
   ([:1055](../js/webrtc-babycam.js), [:1181](../js/webrtc-babycam.js)).
10. `ontrack` builds `remoteStream`, dispatches `remotestream`
    ([:1072](../js/webrtc-babycam.js)).
11. `onconnectionstatechange === 'connected'` → status `connected`, dispatch
    `connected`, extend deadline `RENDERING_TIMEOUT_MS` ([:1042](../js/webrtc-babycam.js)).

**Requirements**

- **REQ-CALL-1** — Starting a call must first end every existing call for the session
  (one active connection at a time) ([:827](../js/webrtc-babycam.js)).
...
- **REQ-CALL-5** — **Stale-event guard:** every peer/signaling callback must ignore
  ...
  (`isStale()`) ([:986](../js/webrtc-babycam.js), [:1202](../js/webrtc-babycam.js)).
...
- **REQ-CALL-6** — Offer creation requests an ICE restart and disables VAD; offer
  send and ICE send each extend the signaling deadline ([:1004](../js/webrtc-babycam.js)).
- **REQ-CALL-7** — `ontrack` must filter tracks against config (drop audio when
  ...
  `ended`, nulling the stream when empty ([:1082](../js/webrtc-babycam.js)).
...
1. `sessionEvent('remotestream'|'connected')` → `loadRemoteStream()` /
   `unloadRemoteStream()` ([:2038](../js/webrtc-babycam.js), [:2085](../js/webrtc-babycam.js)).
2. `loadRemoteStream` sets `media.srcObject` (bumping `playGen`), marks `loaded`, and
   calls `playMedia()` if streaming and not already active ([:3101](../js/webrtc-babycam.js)).
3. Media element events drive the view state machine (`mediaEvent`,
   [:3363](../js/webrtc-babycam.js)):
...
- **REQ-MEDIA-1** — Loading the *same* `srcObject` again must be a no-op; a new stream
  ...
  ([:3106](../js/webrtc-babycam.js), [:3299](../js/webrtc-babycam.js)).
...
- **REQ-MEDIA-3** — On `play`, if frames do not render within `RENDERING_TIMEOUT_MS`
  ...
  ([:3418](../js/webrtc-babycam.js)). This catches "connected but black" streams.
- **REQ-MEDIA-4** — `unloadRemoteStream` must clear all media attributes, stop the
  frame monitor, clear timers, and null `srcObject` ([:3123](../js/webrtc-babycam.js)).
- **REQ-MEDIA-5** — The created media element is always a muted, autoplay,
  playsinline `<video>` with native controls off ([:3342](../js/webrtc-babycam.js)).

---

## 7. Pathway: Signaling transport contract

`openSignalingChannel` ([:1112](../js/webrtc-babycam.js)) selects an implementation by
`config.url_type` and wires `oncandidate`/`onanswer`/`onerror`/`ontrace`, then
`open(SIGNALING_TIMEOUT_MS)`.

| `url_type` | Class | Transport |
|------------|-------|-----------|
| `go2rtc` | `Go2RtcSignalingChannel` | WebSocket `…/api/ws?src=…` ([:1120](../js/webrtc-babycam.js)) |
| `webrtc-babycam` | `Go2RtcSignalingChannel` | HA-signed `/api/webrtc/ws` proxy ([:1130](../js/webrtc-babycam.js)) |
| `webrtc-camera` | `Go2RtcSignalingChannel` | HA-signed `/api/webrtc/ws` ([:1146](../js/webrtc-babycam.js)) |
| `whep` | `WhepSignalingChannel` | HTTP POST offer / PATCH trickle-ICE ([:1160](../js/webrtc-babycam.js)) |
| `rtsptoweb` | `RTSPtoWebSignalingChannel` | HTTP POST form ([:1168](../js/webrtc-babycam.js)) |
...
- **REQ-SIG-2** — `webrtc-babycam`/`webrtc-camera` must obtain a signed path via
  ...
  ([:1137](../js/webrtc-babycam.js)).
- **REQ-SIG-3** — `Go2RtcSignalingChannel.open` must time out after the given ms,
  closing the socket and rejecting if not open ([:3781](../js/webrtc-babycam.js)).
- **REQ-SIG-4** — go2rtc messages map: `webrtc/offer`→send, `webrtc/answer`→`onanswer`,
  ...
  + close ([:3857](../js/webrtc-babycam.js)).
- **REQ-SIG-5** — WHEP must POST SDP (expect `201`, capture `E-Tag`) and PATCH ICE
  ...
  offer ufrag/pwd/media lines ([:3701](../js/webrtc-babycam.js), [:3677](../js/webrtc-babycam.js)).
- **REQ-SIG-6** — All HTTP signaling must enforce `timeout` via `AbortController` and
  surface timeouts through `onerror` ([:3705](../js/webrtc-babycam.js), [:3989](../js/webrtc-babycam.js)).
- **REQ-SIG-7** — If no channel can be built the session sets `error` and aborts the
  call ([:1174](../js/webrtc-babycam.js)).
...
video is not actively playing. `imageLoop` ([:428](../js/webrtc-babycam.js)) +
`fetchImage` ([:1267](../js/webrtc-babycam.js)).
...
1. `imageLoop` self-schedules at the smallest `image_interval` across attached cards,
   **phase-aligned** by a hash of the session key so multiple cameras stagger their
   fetches ([:396](../js/webrtc-babycam.js), [:415](../js/webrtc-babycam.js)).
2. Each tick fetches an image **only if no card is currently playing video**
   ([:446](../js/webrtc-babycam.js)).
3. `fetchImage` resolves the URL (HA `entity_picture`, else `config.image_url`),
   ...
   which dispatches `image` ([:1289](../js/webrtc-babycam.js)).
...
- **REQ-IMG-5** — Displayed images visibly age: a CSS animation blurs/fades the still
  ...
  ([:1590](../js/webrtc-babycam.js), [:2604](../js/webrtc-babycam.js)). This signals
  stale data to the viewer.
- **REQ-IMG-6** — Object URLs created for `<img>` must be revoked on replacement and
  after `image_expiry` to avoid leaks ([:2600](../js/webrtc-babycam.js), [:2629](../js/webrtc-babycam.js)).
...
**Flow** ([:2362–:2467](../js/webrtc-babycam.js)):
...
- **REQ-LIVE-1** — Video liveness is tracked frame-accurately via
  ...
  ([:2394](../js/webrtc-babycam.js), [:3473](../js/webrtc-babycam.js)).
- **REQ-LIVE-2** — If no frame/activity arrives within `image_expiry`, the media must
  be marked `stale` (visually blurred, `live` off) ([:2405](../js/webrtc-babycam.js)).
- **REQ-LIVE-3** — Going stale on a video card must defer video and fetch a fresh
  snapshot so the viewer keeps seeing a recent image ([:2432](../js/webrtc-babycam.js)).
- **REQ-LIVE-4** — A real video frame must relieve video pressure (reset backoff)
  ([:2453](../js/webrtc-babycam.js), [:2454](../js/webrtc-babycam.js)).
- **REQ-LIVE-5** — The `live` indicator (blinking dot) is re-triggered each
  ...
  ([:2052](../js/webrtc-babycam.js), `live()` [:3137](../js/webrtc-babycam.js)).
...
1. At construction, probe `canPlayUnmutedAudio()` (silent MP3) to seed
   `WebRTCsession.unmuteEnabled` ([:116](../js/webrtc-babycam.js), [:125](../js/webrtc-babycam.js)).
2. The first document `keydown`/`mousedown`/`touchstart` calls `enableUnmute()`,
   broadcasting `unmuteEnabled` ([:1966](../js/webrtc-babycam.js)).
3. `playMedia` ([:3259](../js/webrtc-babycam.js)) decides mute state; if unmute isn't
...
4. On `unmuteEnabled` event, any card with `unmute-pending` unmutes
   ([:2078](../js/webrtc-babycam.js)).

**Requirements**

- **REQ-AUDIO-1** — Playback must never hard-fail on autoplay restrictions: an
  unmuted-play rejection must fall back to muted play ([:3318](../js/webrtc-babycam.js)).
- **REQ-AUDIO-2** — Desired-but-blocked unmuting is remembered via `unmute-pending`
  and applied automatically once a gesture enables it ([:3284](../js/webrtc-babycam.js),
  [:2079](../js/webrtc-babycam.js)).
- **REQ-AUDIO-3** — A successful unmuted play sets the global `unmuteEnabled` true so
  other cards can unmute ([:3307](../js/webrtc-babycam.js)).
- **REQ-AUDIO-4** — `unmuteEnabled` is a global (cross-session) capability flag,
  broadcast to every session on change ([:680](../js/webrtc-babycam.js)).
...
**Flow** (`mediaEvent('pause')`, [:3378](../js/webrtc-babycam.js)):
...
- `onconnectionstatechange` failure → `restartCall` ([:1047](../js/webrtc-babycam.js)).
- `restartCall` ([:546](../js/webrtc-babycam.js)) ends the call, clears the watchdog,
...
- Media `waiting`/`stalled`/`error` mark the media stale and (for video) defer video;
  ...
  ([:3477](../js/webrtc-babycam.js), [:3486](../js/webrtc-babycam.js), [:3524](../js/webrtc-babycam.js)).

**Requirements**

- **REQ-RECON-1** — Any terminal/failed peer connection state must trigger a full
  call restart ([:1047](../js/webrtc-babycam.js)).
...
- **REQ-RECON-3** — `endCall` must fully release resources: close signaling + peer,
  ...
  emit `remotestream:null` + `connected:false` ([:914](../js/webrtc-babycam.js)).
- **REQ-RECON-4** — Renegotiation requested while the signaling channel is unavailable
  must restart the call rather than fail silently ([:996](../js/webrtc-babycam.js)).
...
(`webrtc.<key>.background`) ([:762](../js/webrtc-babycam.js)).

**Key predicate:** `shouldKeepBackgroundAudio` = background on, audio enabled, and no
*visible* video card ([:340](../js/webrtc-babycam.js)).
...
- **REQ-BG-1** — When `shouldKeepBackgroundAudio` holds, video deferral is ignored and
  ...
  ([:369](../js/webrtc-babycam.js), [:3479](../js/webrtc-babycam.js)).
- **REQ-BG-2** — A hidden card stays attached only if it is the designated background
  ...
  stream ([:2895](../js/webrtc-babycam.js), [:2834](../js/webrtc-babycam.js)).
- **REQ-BG-3** — Background state changes persist across reloads and broadcast a
  `background` event ([:766](../js/webrtc-babycam.js)).
- **REQ-BG-4** — Background mode is reachable only when allowed
  ...
  mute → background → mute accordingly ([:3219](../js/webrtc-babycam.js)).
...
**Flow** (`deferVideo` [:367](../js/webrtc-babycam.js),
`relieveVideoPressure` [:356](../js/webrtc-babycam.js)):
...
- **REQ-UI-6** — Double-tap / double-click toggles fullscreen (when enabled); with
  ...
  call ([:2749](../js/webrtc-babycam.js)).
- **REQ-UI-7** — The state icon reflects status: heart-broken (no status), loading
  ...
  (`refreshState` [:2469](../js/webrtc-babycam.js)).
...
- **REQ-STATS-1** — Stats sampling occurs only when enabled, on the watchdog tick
  ([:492](../js/webrtc-babycam.js), [:332](../js/webrtc-babycam.js)).
- **REQ-STATS-2** — `updateStatistics` derives received bytes/s, fps, and a "render
  ...
  ([:241](../js/webrtc-babycam.js)). Render quality compares decoded vs expected
  frames (configured/estimated fps).
- **REQ-STATS-3** — The computed header is published to `state.statistics` and shown
  in the card header when stats are visible ([:2480](../js/webrtc-babycam.js)).

---

## 17. Global controls & keyboard shortcuts

`globalInit` ([:1942](../js/webrtc-babycam.js)) installs document listeners once.
...
- **REQ-GLOBAL-1** — Keyboard toggles (with Shift): `T` global mute, `D` global debug,
  ...
  ([:1946](../js/webrtc-babycam.js), [:692](../js/webrtc-babycam.js)).
- **REQ-GLOBAL-2** — `?debug`/`?stats` URL params seed the global debug/stats flags
  ([:20](../js/webrtc-babycam.js)).
- **REQ-GLOBAL-3** — Debug mode reveals a translucent in-card log that mirrors session
  traces; tracing auto-enables when debug is shown ([:1458](../js/webrtc-babycam.js),
  [:2730](../js/webrtc-babycam.js)).
- **REQ-GLOBAL-4** — The element registers as `<webrtc-babycam>` and pushes a Lovelace
  card descriptor onto `window.customCards` ([:3547](../js/webrtc-babycam.js)).
